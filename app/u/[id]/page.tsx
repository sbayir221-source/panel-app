...
const parseBBCode = (text: string = "") => {
  let html = text
    ...
    // 🆕 YENİ: Yatay Çizgi (Horizontal Rule)
    .replace(/\[hr\]/g, "<hr class='my-6 border-zinc-800/80' />")
    
    // 🆕 YENİ: Linkler (URL) - Hem [url=link]Metin[/url] hem de düz [url]link[/url] için
    .replace(/\[url=(.*?)\](.*?)\[\/url\]/g, "<a href='$1' target='_blank' class='text-emerald-500 hover:underline font-bold transition-all'>$2</a>")
    .replace(/\[url\](.*?)\[\/url\]/g, "<a href='$1' target='_blank' class='text-emerald-500 hover:underline font-bold transition-all'>$1</a>")
...
export default function UserProfile() {
  ...
  // URL ne olursa olsun ID'yi tarayıcı seviyesinde garantiye al
  ...
  // Giriş Yapanın Kendi Profili (Ayarlar İçin)
  useEffect(() => {
  ...
  // Profil Verilerini Çekme (Tüm kilitlenmeleri kıran ana parça)
  useEffect(() => {
    if (!profileId) return;
  ...
  if (!profileId) return <div className="min-h-screen bg-[#09090b] flex items-center justify-center text-zinc-600 font-black text-xs uppercase tracking-widest italic">Rota Bulunuyor...</div>;
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 font-sans selection:bg-zinc-500/30">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="bg-zinc-900/40 border border-zinc-800/50 p-10 rounded-[3rem] mb-12 flex flex-col md:flex-row items-center md:items-start gap-8 shadow-2xl relative overflow-hidden">
          ...
          <div className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl font-black uppercase text-white shadow-inner flex-shrink-0 mt-2 ${themeBg}`}>{displayNickname[0]}</div>
              ...
              <article key={post.id} className="bg-zinc-900/20 border border-zinc-800/50 rounded-[2.5rem] overflow-hidden hover:bg-zinc-900/30 transition-all group">
                ...
                <div className="p-8">
                  <div className="flex justify-between mb-4">
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${themeText} ${themeBgTint}`}>{post.category || "Genel"}</span>
                    <span className="text-[9px] font-black text-zinc-600 uppercase flex items-center gap-1"><Calendar size={12}/> {new Date(post.createdAt).toLocaleDateString('tr-TR')}</span>
                  </div>
                  <h3 className="text-xl font-bold mb-4">{post.title}</h3>
                  <div className="text-zinc-400 text-sm mb-6" dangerouslySetInnerHTML={parseBBCode(post.content)} />
                  <div className="flex gap-4 pt-4 border-t border-zinc-800/30 text-xs font-bold text-zinc-600"><span className="flex items-center gap-1"><Heart size={16} className="text-red-500" fill="currentColor"/> {post.likes?.length || 0}</span></div>
                </div>
              </article>
            ))
          )}
